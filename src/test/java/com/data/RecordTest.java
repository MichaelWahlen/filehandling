...
class RecordTest {
	...
	@Test
	@DisplayName("Adding at specific location within bounds")
	void addAtSpecific() {
		row.insertValueAt(1, "Bla");
		assertEquals(row.getAt(0),"Alia");
		assertEquals(row.getAt(1),"Bla");
		assertEquals(row.getAt(2),"Iacta");
		assertEquals(row.getStatus(),DataStatus.UNCHANGED_OK);
	}
	
	@Test
	@DisplayName("Adding at specific location outside of bounds")
	void addAtSpecificOut() {
		row.insertValueAt(6, "Bla");
		assertEquals(row.getAt(0),"Alia");
		assertEquals(row.getAt(1),"Iacta");
		assertEquals(row.getAt(2),"Est");
		assertEquals(row.getAt(6),"Bla");
		assertNull(row.getAt(4));
		assertEquals(row.getStatus(),DataStatus.UNCHANGED_OK);
	}
